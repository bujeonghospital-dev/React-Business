...
function checkEnvFile() {
  ...
  requiredVars.forEach(varName => {
    ...
    const hasValue = value && value.length > 0 && !value.includes('your_') && !value.includes('your-');
  ...
  log('\n━'.repeat(60), 'cyan');
  log('\n📊 สรุป:', 'cyan');
  log(`   Required: ${requiredCount}/${requiredVars.length}`, requiredCount === requiredVars.length ? 'green' : 'red');
  ...
  if (hasAllRequired) {
    ...
    if (optionalCount < optionalVars.length) {
      log(`\n⚠️  คำเตือน: Optional variables ขาด ${optionalVars.length - optionalCount} ตัว`, 'yellow');
    ...
    log('   • Access Token: https://developers.facebook.com/tools/explorer/', 'blue');
    log('   • Ad Account ID: https://business.facebook.com/ → Ad Accounts', 'blue');
