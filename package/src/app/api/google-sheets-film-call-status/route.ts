...
export async function GET(request: NextRequest) {
  try {
    ...
    if (rows[0].some((cell: string) => /^[A-Z]{1,3}$/.test(String(cell || "").trim()))) {
      headers = rows[1];
    ...
    const findCol = (terms: string[]) => headers.findIndex((h: string) =>
    ...
    let productIdx = findCol(["ผลิตภัณฑ์", "product"]);
    ...
    if (phoneIdx === -1) {
      return NextResponse.json({ success: false, error: "Phone column not found", availableHeaders: headers }, { status: 400 });
    }
    ...
    dataRows.forEach((row, idx) => {
      ...
      const product = productIdx !== -1 ? (row[productIdx]?.toString().trim() || "") : "";
      const remarks = remarksIdx !== -1 ? (row[remarksIdx]?.toString().trim() || "") : "";

      if (status) statuses.add(status);
      if ((statusIdx === -1 || status === "อยู่ระหว่างโทรออก") && /^0\d{8,9}$/.test(phone)) {
        calls.push({
          id: `film-${idx + 2}`,
    ...
    const data = {
      ...
      debug: {
        ...
        columns: { status: statusIdx, phone: phoneIdx, name: nameIdx, product: productIdx, remarks: remarksIdx },
